...
Observed.MNTD <- function(TREE,COMM){
    print("Observed MNTD")
    MatchedTips <- match(TREE$tip.label, COMM)
    SortedData <- COMM[,][MatchedTips,]
    ...
    #SES_MNTD_MOD
    SES <- SES_MNTD_MOD(comm, phydist, null.model = "taxa.labels", abundance.weighted = FALSE, runs = 10000)
    SES_reg <- ses.mntd(comm, phydist, null.model = "taxa.labels", abundance.weighted = FALSE, runs = 10000)
    ...
    return(list(OUTPUT,SES_MNTD_MOD_ratio))
}