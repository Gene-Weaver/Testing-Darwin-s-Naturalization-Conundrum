...
#       ***This version of the script is designed for ONLY the observed tree, not the 1000 tree file
#       ***The loop for TREE loop was removed
...
#####################################################
TREE_RAW_0 <- read.nexus("./NEON_MCC_wgymnoclade_NEW.tre")
...
IGNORE <- data.frame(c("Macfadyena_unguis.cati","Smilax_bona.nox","Abies_alba",
                       ...
                       "Lygodium_japonicum"))
TREE_RAW_0 <- drop.tip(TREE_RAW_0,as.character(IGNORE[,1]))
...
for(i in 1:length(SITENAMES[,1])){
    ...
    tree_DEFINE <- TREE_RAW_0
    
    # Names of all species in tree
    Tree_Names_DEFINE <- as.character(tree_DEFINE$tip.label)
    ...
    temp_out <- Observed.PAE(TREE,TREE_N_ONLY,COMM,ABUND,ABUND_N_ONLY)
    ...
    while (rep <= numReps){
        ...
        SIM_MNTD <- SIM_MNTD[1]
        MNTD_SIM_EXPORT <- rbind(MNTD_SIM_EXPORT,cbind(rbind(rep,rep),SIM_MNTD[2]))
        print("PAE End")
        ...
        # For consistency, the ratio is calculated just as with mntd and pae
        #PD_SIM_RATIO <- SIM_PAE[[2]][2]/SIM_PAE[[2]][10]
    ...
    # Filenames PAE
    filename_TREE_P_VALUE <- paste("./",DIRECTORY,CURRENT_SITE,"_OBSERVED_PAE_Simulation_PValues.csv",sep="")
    filename_EXPORT_SESPD_OBSERVED <- paste("./",DIRECTORY,CURRENT_SITE,"_OBSERVED_SESPD_Observed.csv",sep="")
    filename_EXPORT_SESPD_GROUPS <- paste("./",DIRECTORY,CURRENT_SITE,"_OBSERVED_SESPD_Groups.csv",sep="")
    filename_EXPORT_PAE_ALL <- paste("./",DIRECTORY,CURRENT_SITE,"_OBSERVED_PAE_Observed.csv",sep="")
    
    # Filename PD p-values
    filename_TREE_P_VALUE_PD <- paste("./",DIRECTORY,CURRENT_SITE,"_OBSERVED_SESPD_Simulation_PValues.csv",sep="")
    ...
    # Export MNTD Simulation
    filename_MNTD_SIM_EXPORT <- paste("./",DIRECTORY,CURRENT_SITE,"_MNTD_12MILL_Simulation.csv",sep="")
    write.csv(MNTD_SIM_EXPORT,filename_MNTD_SIM_EXPORT,row.names=FALSE)
    ...
    # Filenames MNTD 
    filename_TREE_P_VALUE_MNTD <- paste("./",DIRECTORY,CURRENT_SITE,"_OBSERVED_MNTD_Simulation_PValues.csv",sep="")
    filename_EXPORT_MNTD_OBSERVED <- paste("./",DIRECTORY,CURRENT_SITE,"_OBSERVED_MNTD_Observed.csv",sep="")
    # Write MNTD
